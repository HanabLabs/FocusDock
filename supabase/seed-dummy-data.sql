-- Dummy data migration for riiriricon@gmail.com
-- This script creates dummy data for a paid user with GitHub commits, work sessions, and Spotify sessions
-- Run this script in Supabase SQL Editor (RLS policies are bypassed in SQL Editor)

-- First, ensure the user exists in auth.users (this should be done via Supabase Auth)
-- Then update/create the user profile with paid subscription
DO $$
DECLARE
  user_id_val UUID;
BEGIN
  ...
  IF user_id_val IS NULL THEN
    RAISE EXCEPTION 'User with email riiriricon@gmail.com not found in auth.users. Please create the user first via Supabase Auth.';
  END IF;

  -- Insert or update user profile with paid subscription
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    github_username = 'riiriricon',
    spotify_connected = true,
    updated_at = NOW();
END $$;

-- Function to generate GitHub commits dummy data (last 30 days)
DO $$
DECLARE
  user_id_val UUID;
  current_date DATE;
...
BEGIN
  ...
  -- Delete existing commits for this user
  DELETE FROM public.github_commits WHERE user_id = user_id_val;
...
-- Function to generate work sessions dummy data (last 30 days)
DO $$
DECLARE
  user_id_val UUID;
  session_date DATE;
...
BEGIN
  ...
  -- Delete existing work sessions for this user
  DELETE FROM public.work_sessions WHERE user_id = user_id_val;

  -- Generate work sessions for last 30 days
  FOR i IN 0..29 LOOP
...
-- Function to generate Spotify sessions dummy data (last 30 days)
DO $$
DECLARE
  user_id_val UUID;
  session_date DATE;
...
BEGIN
  ...
  -- Delete existing Spotify sessions for this user
  DELETE FROM public.spotify_sessions WHERE user_id = user_id_val;

  -- Generate Spotify sessions for last 30 days
  FOR i IN 0..29 LOOP
    session_date := CURRENT_DATE - i;
